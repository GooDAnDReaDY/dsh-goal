...
import { getTemplatesCatalogue } from './engine-templates.js';
...
export function isTrustedCaller(req) {
  ...
  if (secSite === 'same-origin' || secSite === 'same-site' || secSite === 'none') return true;
  ...
  // Fallback for tests or local CLI without socket where host header targets loopback
  if (!remote && host) {
...
export function isPathInsideOrEqual(parent, child) {
  if (!parent || !child) return false;
  const rel = path.relative(path.resolve(parent), path.resolve(child));
  return !rel || (!rel.startsWith('..') && !path.isAbsolute(rel));
}

export function getSafeCwd(requestedCwd, allowedRoots = []) {
  const defaultRoot = allowedRoots[0] || process.cwd();
  ...
  try {
    ...
    const isAllowed = roots.some((root) => isPathInsideOrEqual(root, resolved));
    if (!isAllowed) {
      return null;
    }
            ...
            try {
              ...
              switch (action) {
                ...
                case 'rollback_milestone': {
                  ...
                  const sessionGoal = engine.getGoal(sid);
                  const allowedRoots = [process.cwd()];
                  if (sessionGoal?.cwd) allowedRoots.push(sessionGoal.cwd);
