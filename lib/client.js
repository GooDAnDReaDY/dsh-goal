window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    // Контракт вкладки (ядро rc.1): карточка сама рисует внутренности и читает
    // своё пространство через ctx.settingsScope.bind({ namespace }). Снимок
    // пространства носит статус (loading / unavailable / ready) — решать по
    // статусу, а не по значению; пока namespace не ready, карточка не рисуется.
    function GoalSettingsCard(props) {
      ...
      useEffect(() => {
        ...
        // Как CardForm ядра: staged-черновики переживают обновления снимка
        // (внешние записи, commit документа, reconnect) — стираются только
        // после удачного сохранения или явного «Сбросить».
      ...
      // Все хуки объявлены выше ранних возвратов!
      if (!snap || snap.status !== 'ready') return null;

      const value = snap.value || {};
      const baseMaxIter = value.maxIterations ?? 25;
      const current = draft || {
        maxIterations: String(baseMaxIter),
        autoDrive: value.autoDrive ?? true,
        enableSound: value.enableSound ?? true,
      };
      const iterNum = Number(current.maxIterations);
      const invalid = !Number.isFinite(iterNum) || iterNum < 1;
      const disabled = !snap.writable || saving;
      const dirty =
        draft !== null &&
        (iterNum !== Number(baseMaxIter) ||
          current.autoDrive !== (value.autoDrive ?? true) ||
          current.enableSound !== (value.enableSound ?? true));

      const edit = (field, fieldValue) => {
        setFailed(false);
        setDraft({ ...current, [field]: fieldValue });
      };

      // Писать все изменённые поля, а не до первой ошибки; результат сверять
      // обратным чтением пользовательского слоя, как делает ядро (CardForm).
      const save = async () => {
        if (!scope || disabled || invalid || !dirty) return;
        setSaving(true);
        setFailed(false);
        const writes = [];
        if (iterNum !== Number(baseMaxIter)) writes.push(['maxIterations', iterNum]);
        if (current.autoDrive !== (value.autoDrive ?? true)) writes.push(['autoDrive', current.autoDrive]);
        if (current.enableSound !== (value.enableSound ?? true)) writes.push(['enableSound', current.enableSound]);
        let landed = true;
        ...
        isOpen
          ? React.createElement(
              ...
              React.createElement(
                ...
                React.createElement('input', {
                  type: 'checkbox',
                  checked: current.autoDrive === true,
                  disabled,
              ...
              React.createElement(
                ...
                React.createElement('input', {
                  type: 'checkbox',
                  checked: current.enableSound === true,
                  disabled,
    ...
    module.exports.apply = function apply(ctx) {
      ...
      // Пространство настроек плагина: бинд один раз, карточка читает снимок
      // и пишет значения через этот scope.
      const settingsScope =
      ...
      // 2. Регистрация карточки настроек (key = пространство настроек)
      ctx.slots?.register?.(
        {
