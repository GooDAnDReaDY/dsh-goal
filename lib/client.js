window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const CSS_STYLES = `
      ...
      .dsh-goal-preplan-card {
        padding: 12px;
        border-radius: 8px;
        background: var(--dsw-alias-bg-layer-2);
        border: 1px solid var(--dsw-alias-state-info-primary, var(--dsw-alias-border-l2));
        margin-bottom: 12px;
      }

      .dsh-goal-activity-feed {
        padding: 10px 12px;
        border-radius: 8px;
        background: var(--dsw-alias-bg-layer-2);
        border: 1px solid var(--dsw-alias-border-l2);
      ...
      .dsh-goal-activity-dot {
        width: 6px;
        height: 6px;
        border-radius: 50%;
        background: var(--dsw-alias-state-info-primary, var(--dsw-alias-primary));
        flex-shrink: 0;
      }

      .dsh-goal-activity-time {
        font-size: 10px;
        color: var(--dsw-alias-label-tertiary);
        flex-shrink: 0;
      }

      .dsh-goal-activity-text {
        overflow: hidden;
        text-overflow: ellipsis;
        white-space: nowrap;
        flex: 1;
      ...
      .dsh-goal-lock-badge {
        ...
        padding: 1px 6px;
        border-radius: 4px;
        background: var(--dsw-alias-state-warning-bg, var(--dsw-alias-bg-layer-4));
        color: var(--dsw-alias-state-warning-primary);
        border: 1px solid var(--dsw-alias-state-warning-primary);
        margin-left: 6px;
      }
    ...
    // Synthesize audio chimes via Web Audio API (Item 6) & soundScheme (#92)

    function playGoalChime(isSuccess = true, scheme = 'default') {
      ...
      try {
        ...
        if (actx.state === 'suspended') {
          actx.resume().catch((err) => { console.warn('[dsh-goal] Fetch templates error:', err); });
        }
        const now = actx.currentTime;
        if (scheme === 'retro') {
          const notes = isSuccess ? [330, 440, 554, 659] : [300, 220, 160];
        ...
        if (scheme === 'minimal') {
          ...
          const gain = actx.createGain();
          osc.type = 'triangle';
          osc.frequency.setValueAtTime(isSuccess ? 880 : 330, now);
    ...
    // Voice announcement via Web Speech API (#92)
    function speakGoalAnnouncement(text, enabled = false) {
    ...
    function GoalDetailsModal({ state, onClose, onAction, t }) {
      ...
      const [interveneSent, setInterveneSent] = useState(false);
      ...
      return React.createElement(
        ...
        React.createElement(
          ...
          React.createElement(
            ...
            // Smart Pre-planning Draft Card (#85)
            state.preplan && state.preplan.active && Array.isArray(state.preplan.draftMilestones) && state.preplan.draftMilestones.length > 0
              ? React.createElement(
                  'div',
                  { className: 'dsh-goal-preplan-card' },
                  React.createElement('div', { style: { fontWeight: 600, fontSize: 13, marginBottom: 4, display: 'flex', alignItems: 'center', gap: 6 } },
                    React.createElement('span', null, '📋'),
                    React.createElement('span', null, t('preplanTitle') || 'Smart Pre-plan Ready')
                  ),
                  React.createElement('div', { style: { fontSize: 12, color: 'var(--dsw-alias-label-secondary)', marginBottom: 8 } },
                    `Generated ${state.preplan.draftMilestones.length} draft milestone(s). Review and approve to start.`
                  ),
                  React.createElement('div', { style: { display: 'flex', gap: 8 } },
                    React.createElement('button', {
                      type: 'button',
                      className: 'dsh-goal-btn dsh-goal-btn-primary',
                      onClick: () => onAction('preplan_approve', { sessionId: state.sessionId, approvedMilestones: state.preplan.draftMilestones })
                    }, t('preplanApprove') || 'Approve & Start Plan')
                  )
                )
              : null,

            // Pause & Intervene Card (#83)
            state.state === 'PAUSED'
              ? React.createElement(
                  'div',
                  { className: 'dsh-goal-intervene-card' },
                  React.createElement('div', { style: { fontWeight: 600, fontSize: 13, marginBottom: 4, display: 'flex', alignItems: 'center', gap: 6 } },
                    React.createElement('span', null, '⏸️'),
                    React.createElement('span', null, t('interveneTitle') || 'Pause & Intervene')
                  ),
                  React.createElement('div', { style: { fontSize: 12, color: 'var(--dsw-alias-label-secondary)', marginBottom: 8 } },
                    t('interveneDesc') || 'Goal is paused. Steer agent behavior with direct guidance before resuming.'
                  ),
                  React.createElement('textarea', {
                    className: 'dsh-goal-card-input',
                    style: { width: '100%', minHeight: 60, fontSize: 12, resize: 'vertical', marginBottom: 8 },
                    placeholder: t('interveneDirectivePlaceholder') || 'Enter directive instructions for the agent...',
                    value: interveneText,
                  ...
                  React.createElement('div', { style: { display: 'flex', gap: 8, justifyContent: 'flex-end' } },
                    React.createElement('button', {
                      type: 'button',
                      className: 'dsh-goal-btn dsh-goal-btn-primary',
                      disabled: !interveneText.trim() && !interveneSent,
                      onClick: () => {
                        if (interveneText.trim()) {
                          onAction('intervene', { directive: interveneText.trim() });
                          setInterveneSent(true);
                          setTimeout(() => setInterveneSent(false), 2000);
                        }
                      }
                    }, interveneSent ? (t('interveneApplied') || '✅ Applied') : (t('interveneApply') || 'Apply Directive & Resume'))
                  )
                )
              : null,
                    ...
                    React.createElement(
                      ...
                      React.createElement(
                        ...
                        React.createElement(
                          ...
                          `${i + 1}. ${m.title}`,
                            Array.isArray(m.dependsOn) && m.dependsOn.some((depId) => {
                              const prereq = milestones.find((x) => x.id === depId || x.title === depId);
                              return prereq && prereq.status !== 'completed';
                            })
                              ? React.createElement('span', { className: 'dsh-goal-lock-badge' }, `🔒 ${t('waitingPrereq') || 'waiting for prereq'}`)
                              : null,
            ...
            // Git Branch Action Controls (#86)
            state.gitBranch
              ? React.createElement(
                  'div',
                  { style: { display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8, padding: '8px 10px', borderRadius: 8, background: 'var(--dsw-alias-bg-layer-2)', border: '1px solid var(--dsw-alias-border-l2)', marginTop: 8 } },
                  React.createElement('span', { className: 'dsh-goal-git-badge', style: { margin: 0 } }, `🌿 ${state.gitBranch}`),
                  (isCompleted || state.state === 'CANCELLED')
                    ? React.createElement(
                        'div',
                        ...
                        React.createElement('button', {
                          ...
                          onClick: () => onAction('branch_action', { action: 'merge', sessionId: state.sessionId })
                        }, t('mergeBranch') || 'Merge Branch'),
                        React.createElement('button', {
                          type: 'button',
                          className: 'dsh-goal-btn',
                          style: { fontSize: 11, padding: '2px 8px' },
                          onClick: () => onAction('branch_action', { action: 'discard', sessionId: state.sessionId })
                        }, t('discardBranch') || 'Discard Branch')
                      )
                    : null
                )
              : null,

            // Live Activity Mini-Feed (#84)
            state.enableLiveActivityFeed !== false && Array.isArray(state.activityFeed) && state.activityFeed.length > 0
              ? React.createElement(
                  'div',
                  { className: 'dsh-goal-activity-feed' },
                  React.createElement('div', { style: { fontWeight: 600, fontSize: 12, marginBottom: 6, color: 'var(--dsw-alias-label-primary)', display: 'flex', alignItems: 'center', gap: 6 } },
                    React.createElement('span', null, '⚡'),
                    React.createElement('span', null, t('liveActivityTitle') || 'Live Activity Feed'),
                    React.createElement('span', { style: { fontSize: 10, color: 'var(--dsw-alias-label-tertiary)', fontWeight: 400 } }, `(${state.activityFeed.length})`)
                  ),
                  React.createElement(
                    'div',
                    { className: 'dsh-goal-activity-list' },
                    state.activityFeed.slice(-6).reverse().map((act, actIdx) =>
                      React.createElement('div', { key: act.id || actIdx, className: 'dsh-goal-activity-row' },
                        React.createElement('span', { className: 'dsh-goal-activity-dot' }),
                        React.createElement('span', { className: 'dsh-goal-activity-text' }, act.details || act.tool || act.type)
                      )
                    )
                  )
                )
            ...
            // Post-Goal Retrospective Card (#88)
            isCompleted && state.retrospective
              ? React.createElement(
                  'div',
                  { className: 'dsh-goal-retrospective-card' },
                  React.createElement('div', { style: { fontWeight: 600, fontSize: 13, marginBottom: 8, display: 'flex', alignItems: 'center', gap: 6 } },
                    React.createElement('span', null, '📊'),
                    React.createElement('span', null, t('retrospectiveTitle') || 'Goal Retrospective')
                  ),
                  React.createElement(
                    'div',
                    { style: { display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 8, fontSize: 11 } },
                    React.createElement('div', null,
                      React.createElement('div', { style: { color: 'var(--dsw-alias-label-tertiary)' } }, 'Duration'),
                      React.createElement('div', { style: { fontWeight: 600 } }, state.retrospective.formattedDuration || state.formattedElapsed || '0s')
                    ),
                    React.createElement('div', null,
                      React.createElement('div', { style: { color: 'var(--dsw-alias-label-tertiary)' } }, 'Tokens'),
                      React.createElement('div', { style: { fontWeight: 600 } }, state.retrospective.tokensTotal ? `${state.retrospective.tokensTotal}` : '—')
                    ),
                    React.createElement('div', null,
                      React.createElement('div', { style: { color: 'var(--dsw-alias-label-tertiary)' } }, 'Files Modified'),
                      React.createElement('div', { style: { fontWeight: 600 } }, `${state.retrospective.filesModifiedCount || 0}`)
                    ),
                    React.createElement('div', null,
                      React.createElement('div', { style: { color: 'var(--dsw-alias-label-tertiary)' } }, 'Tool Calls'),
                      React.createElement('div', { style: { fontWeight: 600 } }, `${state.retrospective.toolCallsCount || 0}`)
                    ),
                    React.createElement('div', null,
                      React.createElement('div', { style: { color: 'var(--dsw-alias-label-tertiary)' } }, 'Stall Recoveries'),
                      React.createElement('div', { style: { fontWeight: 600 } }, `${state.retrospective.stallsEncountered || 0}`)
                    ),
                    React.createElement('div', null,
                      React.createElement('div', { style: { color: 'var(--dsw-alias-label-tertiary)' } }, 'Milestones'),
                      React.createElement('div', { style: { fontWeight: 600 } }, `${state.retrospective.milestonesCompleted || 0}/${state.retrospective.milestonesTotal || 0}`)
                    )
                  )
                )
              : null,
    ...
    function QuickLaunchModal({ onClose, onStart, t }) {
      ...
      const [tplInputs, setTplInputs] = useState({});
      const [loadingTpls, setLoadingTpls] = useState(false);

      const inputRef = useRef(null);

      useEffect(() => {
        if (showTemplates && backendTemplates.length === 0) {
          setLoadingTpls(true);
          fetch('/dsh-goal/templates')
            .then((r) => r.json())
            .then((d) => {
              if (d && Array.isArray(d.templates)) {
                setBackendTemplates(d.templates);
              }
            })
            .catch((err) => { console.warn('[dsh-goal] Fetch templates error:', err); })
            .finally(() => setLoadingTpls(false));
        }
      }, [showTemplates, backendTemplates.length]);
      ...
      const handleApplyTemplate = async () => {
        ...
        } catch (err) { console.warn('[dsh-goal] Apply template error:', err); }
      };
          ...
          React.createElement(
            ...
            React.createElement(
              ...
              React.createElement(
                ...
                [
                  { id: 'quickfix', label: '⚡ Quick Fix', template: 'Fix issue: ' },
                  { id: 'feature', label: '✨ Feature', template: 'Implement feature: ' },
                  { id: 'refactor', label: '♻️ Refactor', template: 'Refactor module: ' },
                  { id: 'repro', label: '🐛 Bug Repro', template: 'Reproduce and resolve bug: ' },
                  { id: 'audit', label: '🔍 Audit', template: 'Deep audit codebase: ' },
                  { id: 'security', label: '🛡️ Security', template: 'Security hardening for: ' },
                ].map((tpl) =>
                  React.createElement(
                    'button',
                    {
                      key: tpl.id,
                      type: 'button',
                      className: 'dsh-goal-chip-btn',
                      onClick: () => setTitle(tpl.template),
                    },
                    tpl.label
                  )
                ),
                React.createElement(
                  'button',
                  {
                    type: 'button',
                    className: 'dsh-goal-chip-btn',
                    style: { fontWeight: 600, borderColor: 'var(--dsw-alias-border-brand, var(--dsw-alias-border-l2))' },
                    onClick: () => setShowTemplates(!showTemplates),
                  },
                  (showTemplates ? '▲ ' : '▼ ') + '📋 ' + (t('templatesTitle') || 'Engineering Templates')
                )
              ),
              showTemplates
                ? React.createElement(
                    ...
                    loadingTpls
                      ? React.createElement('div', { style: { fontSize: 12, color: 'var(--dsw-alias-label-secondary)' } }, 'Loading templates…')
                      : React.createElement(
                          'div',
                          null,
                          React.createElement(
                            ...
                            backendTemplates.map((bt) =>
                              React.createElement(
                                'button',
                                {
                                  key: bt.id,
                                  type: 'button',
                                  className: `dsh-goal-chip-btn ${selectedTpl?.id === bt.id ? 'active' : ''}`,
                                  style: selectedTpl?.id === bt.id ? { fontWeight: 600, border: '1px solid var(--dsw-alias-accent-primary)' } : {},
                                  onClick: () => {
                                    setSelectedTpl(bt);
                                    const initial = {};
                                    (bt.inputs || []).forEach((inp) => {
                                      initial[inp.key] = inp.defaultValue || '';
                                    });
                                    setTplInputs(initial);
                                  },
                                },
                                bt.title
                              )
                          ...
                          selectedTpl
                            ? React.createElement(
                                'div',
                                { style: { borderTop: '1px solid var(--dsw-alias-border-l1)', paddingTop: 8 } },
                                React.createElement('div', { style: { fontSize: 12, color: 'var(--dsw-alias-label-secondary)', marginBottom: 6 } }, selectedTpl.description),
                                (selectedTpl.inputs || []).map((inp) =>
                                  React.createElement(
                                    'div',
                                    { key: inp.key, style: { marginBottom: 6 } },
                                    React.createElement('label', { style: { fontSize: 11, display: 'block', marginBottom: 2 } }, inp.label),
                                    React.createElement('input', {
                                      type: 'text',
                                      className: 'dsh-goal-card-input',
                                      style: { width: '100%', boxSizing: 'border-box', fontSize: 12, padding: '4px 8px' },
                                      placeholder: inp.placeholder || '',
                                      value: tplInputs[inp.key] || '',
                                      onChange: (e) => setTplInputs({ ...tplInputs, [inp.key]: e.target.value }),
                                    })
                                  )
                                ),
                                React.createElement(
        ...
        if (nextState && lastStateRef.current) {
          ...
          if (prev.state !== nextState.state && nextState.enableSound !== false) {
            ...
            if (nextState.state === 'COMPLETED') {
              ...
              playGoalChime(true, nextState.soundScheme || 'default');
              speakGoalAnnouncement('Goal completed: ' + (nextState.title || ''), Boolean(nextState.enableVoiceAnnouncements));

            } else if (nextState.state === 'FAILED') {

              playGoalChime(false, nextState.soundScheme || 'default');
              speakGoalAnnouncement('Goal failed: ' + (nextState.title || ''), Boolean(nextState.enableVoiceAnnouncements));
                  ...
                  : React.createElement(
                      ...
                      React.createElement(
                        ...
                        React.createElement(
                          ...
                          React.createElement('span', { style: { fontSize: 12, color: 'var(--dsw-alias-label-secondary)' } }, updateState.currentVersion ? 'v' + updateState.currentVersion : 'v0.2.12'),
    ...
    const LOCALES = {
      ...
      en: {
        ...
        interveneTitle: 'Pause & Intervene',
        interveneDesc: 'Goal is paused. Steer agent behavior with direct guidance before resuming.',
        interveneDirectivePlaceholder: 'Enter directive instructions for the agent...',
        interveneApply: 'Apply Directive & Resume',
        interveneApplied: 'Directive Applied',
        liveActivityTitle: 'Live Activity Feed',
        preplanTitle: 'Smart Pre-plan Ready',
        preplanApprove: 'Approve & Start Plan',
        mergeBranch: 'Merge Branch',
        discardBranch: 'Discard Branch',
        retrospectiveTitle: 'Goal Retrospective',
        templatesTitle: 'Goal Templates',
        waitingPrereq: 'waiting for prereq',
      ...
      zh: {
        ...
        interveneTitle: '暂停并干预',
        interveneDesc: '目标已暂停。在恢复前向智能体发送直接指引。',
        interveneDirectivePlaceholder: '输入给智能体的指导指令...',
        interveneApply: '应用指引并继续',
        interveneApplied: '已应用干预',
        liveActivityTitle: '实时动态',
        preplanTitle: '智能预规划就绪',
        preplanApprove: '批准并启动计划',
        mergeBranch: '合并分支',
        discardBranch: '放弃分支',
        retrospectiveTitle: '目标复盘报告',
        templatesTitle: '目标模板',
        waitingPrereq: '等待前置依赖',
    ...
    module.exports.apply = function apply(ctx) {
      if (typeof ctx.effect === 'function') {
        ctx.effect(() => {
          ...
          const configForms =
            ctx.configForms && typeof ctx.configForms.get === 'function'
