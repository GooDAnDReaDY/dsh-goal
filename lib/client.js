window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    // --- ВЕРХНЯЯ ПАНЕЛЬ ЦЕЛИ (STICKY TOP BANNER) ---
      ...
      return React.createElement(
        ...
        React.createElement(
          ...
          React.createElement(
            ...
            React.createElement('span', { className: 'dsh-goal-icon' }, React.createElement(IconTarget, { size: 16 })),
    ...
    module.exports.apply = function apply(ctx) {
      ...
      // 1. Регистрация Sticky Top Banner в слот conversation.session.header (rc.1)
      registerSlotSafe(
      ...
      // Фоллбеки для других слотов
      registerSlotSafe(
        'conversation.header',
        {
          name: 'conversation.header',
          id: '@goodandready/dsh-goal:header',
          order: 10,
          locale: NS,
          inject: () => ({ ctx }),
        },
      ...
      registerSlotSafe(
        ...
        {
          name: 'session.header',
          id: '@goodandready/dsh-goal:session-header',
          order: 10,
          locale: NS,
          inject: () => ({ ctx }),
        },
