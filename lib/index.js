...
export function apply(ctx, config = {}) {
  ...
  const engine = new GoalEngine({
    ...
    soundScheme: currentSettings.soundScheme,
    enableVoiceAnnouncements: currentSettings.enableVoiceAnnouncements,
    storagePath,
    logger,
  });
  ...
  const stopRunningAgents = (sessionId = 'default') => {
    ...
    if (specificAgent && typeof specificAgent.cancel === 'function') {
      try {
        ...
        logger.warn('Failed to cancel specific agent for session:', err);
      }
    }
    ...
    for (const ag of runningAgents) {
      try {
        ...
        logger.warn('Failed to cancel running agent:', err);
      }
    }
    ...
    if (lastActiveAgent && lastActiveAgent.status === 'running' && typeof lastActiveAgent.cancel === 'function') {
      ...
      if (!lastSid || lastSid === sid || sid === 'default') {
        try {
          ...
          logger.warn('Failed to cancel lastActiveAgent:', err);
        }
      }
    }
  };
  ...
  const resumeActiveAgent = (promptText, sessionId = 'default') => {
    ...
    if (target && typeof target.followup === 'function') {
      try {
        ...
        logger.warn('Failed to resume agent:', err);
      }
    }
    return false;
  };
  ...
  const getConfig = () => {
    if (settingsScope) {
      ...
      let live = null;
      if (snap) {
        if (snap.status === 'ready' || snap.status === 'writable' || snap.status === undefined) {
          live = snap.value || (typeof settingsScope.get === 'function' ? settingsScope.get() : null);
        }
      } else if (typeof settingsScope.get === 'function') {
        live = settingsScope.get();
      }
  ...
  const applySettings = () => {
    ...
    engine.updateConfig({
      ...
      enableLiveActivityFeed: live.enableLiveActivityFeed,
      enablePreplanning: live.enablePreplanning,
      autoBranchOnGoalStart: live.autoBranchOnGoalStart,
      enablePostGoalRetrospective: live.enablePostGoalRetrospective,
      autoScaleBudgetNearCompletion: live.autoScaleBudgetNearCompletion,
      enableTemplatesDrawer: live.enableTemplatesDrawer,
      enableMilestoneDependencies: live.enableMilestoneDependencies,
      soundScheme: live.soundScheme,
      enableVoiceAnnouncements: live.enableVoiceAnnouncements,
    });
  };
  ...
  ctx.inject(['commands'], (cctx) => {
    try {
      ...
      logger.warn('Commands register skipped:', err.message);
    }
  });
  ...
  ctx.inject(['systemPrompt'], (pctx) => {
    try {
      if (typeof pctx.systemPrompt?.section === 'function') {
        ...
        let displacedPromptSection = null;
        if (globalSections?.data instanceof Map && globalSections.data.has('tool:goal')) {
          displacedPromptSection = globalSections.data.get('tool:goal');
        ...
        if (typeof unregisterSection === 'function') {
          pctx.effect(() => () => {
            unregisterSection();
            if (displacedPromptSection && globalSections?.data instanceof Map) {
              globalSections.data.set('tool:goal', displacedPromptSection);
            }
          }, 'dsh-goal: system prompt section');
        }
      }
    } catch (err) {
      logger.warn('SystemPrompt section register skipped:', err.message);
    }
  });

  // 4. Tools registration
  registerTools(ctx, { engine, resumeActiveAgent, logger });

  // 5. One-click auto-updater registration with Cordis effect cleanup (Issue #99)
          ...
          if (currentSnap.hasActiveGoal && currentSnap.state === GoalState.RUNNING) {
            ...
            if (target && typeof target.followup === 'function' && target.status !== 'stopped') {
              try {
                ...
                logger.warn('Failed auto-drive followup:', err);
              }
            }
          }
        };
