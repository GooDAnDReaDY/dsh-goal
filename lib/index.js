...
export function apply(ctx, config = {}) {
  ...
  const stopRunningAgents = (reason = 'Goal paused by user') => {
    if (lastActiveAgent && typeof lastActiveAgent.cancel === 'function') {
      try {
        lastActiveAgent.cancel({ kind: 'user', reason });
      } catch (err) {
      ...
      try {
        for (const ag of ctx.agents.list()) {
          if (ag && ag.status === 'running' && typeof ag.cancel === 'function') {
            try {
              ag.cancel({ kind: 'user', reason });
            } catch (_) {}
          }
        }
      } catch (err) {
        console.warn('[dsh-goal] Failed to list/cancel agents:', err);
      }
    }
  };
  ...
  const resumeActiveAgent = (promptText) => {
    ...
    if (lastActiveAgent && typeof lastActiveAgent.followup === 'function') {
      try {
        lastActiveAgent.followup(resumeMsg);
        resumed = true;
      } catch (err) {
        console.warn('[dsh-goal] Failed to resume lastActiveAgent:', err);
      }
    }
    if (!resumed && ctx.agents && typeof ctx.agents.list === 'function') {
      try {
        const list = ctx.agents.list();
        if (list && list.length > 0) {
          const ag = list[list.length - 1];
          if (typeof ag.followup === 'function') {
            ag.followup(resumeMsg);
            lastActiveAgent = ag;
            resumed = true;
          }
        }
      } catch (err) {
        console.warn('[dsh-goal] Failed to resume agent from ctx.agents.list:', err);
      }
    }
    return resumed;
  };
            ...
            try {
              ...
              switch (action) {
                ...
                case 'pause':
                  ...
                  stopRunningAgents(reason || 'Goal paused by user');
                  break;
                case 'resume':
                ...
                case 'cancel':
                  ...
                  stopRunningAgents(reason || 'Goal cancelled by user');
                  break;
                case 'clear':
                  result = engine.clear();
                  stopRunningAgents('Goal cleared by user');
                  break;
