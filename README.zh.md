...
- **严格的多语言规范**: 从 `lib/*.js` 中清除了硬编码的俄语字符串与分支；核心原生支持英文（默认）与中文，俄语由 `@goodandready/dsh-russian-lang` 插件提供。
- **健壮性与错误处理**: 消除了所有空 catch 块，在目标完成与阻塞的上下文延迟中添加了日志记录，并自动清理断开的 SSE 连接。
- **设计系统规范**: 将第三方 CSS 类与硬编码 RGBA 颜色替换为原生 DSH 主题设计变量（`--dsw-alias-*`）。
- **设置卡片状态保护**: 修复了设置作用域未加载时错误显示 `ready` 状态的问题，未就绪时正确显示不可用/加载状态。
