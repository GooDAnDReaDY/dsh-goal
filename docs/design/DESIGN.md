...
4. **2026-09-04 — rc.1-совместимость клиента**: `dsh.client.inject: []` — в таблице модулей ядра 0.1.2-rc.1 нет `dsh-client-runtime`, `dsh-client-ui-slots`, `dsh-client-ui-primitives`; `slots`/`locale`/`settingsScope` — встроенные службы ядра. Условие пересмотра: появление этих модулей в boot-таблице ядра.
