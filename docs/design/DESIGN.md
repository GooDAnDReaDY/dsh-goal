...
### 7.7 Защита от CSRF и изоляция путей запуска (v0.2.9, Issues #78, #79)
- **CSRF & Untrusted LAN Caller Guard (Issue #78)**: Эндпоинт POST /dsh-goal/action проверяет доверие к источнику запроса с помощью isTrustedCaller(req). Запросы из локальной сети (не loopback), не имеющие заголовка Sec-Fetch-Site: same-origin/same-site либо валидных заголовков Origin/Referer, совпадающих с хостом сервера, блокируются с кодом 403 Forbidden.
- **Ограничение рабочего каталога (Issue #79)**: Функция getSafeCwd валидирует 
equestedCwd на предмет нахождения внутри разрешённых корней (process.cwd() и sessionGoal.cwd). Любые попытки выхода за пределы рабочей директории (path traversal, /tmp, /var и т.д.) отклоняются с кодом 400 Bad Request.

## 8. Power Suite v0.2.10 (Issues #83-#92)

### 8.1 Pause & Intervene (Issue #83)
- **Концепция**: Возможность оператора вмешаться в работу агента во время паузы цели (`PAUSED`).
- **Функционал**:
  - В модальном окне деталей при статусе `PAUSED` отображается карточка интервенции с полем директивы и возможностью редактирования плана;
  - По клику «Применить директиву» отправляется действие `intervene` на `/dsh-goal/action`;
  - Директива сохраняется в `goal.userDirective` и при возобновлении (`resume`) внедряется в системный промпт агента с наивысшим приоритетом (`USER INTERVENTION DIRECTIVE`);
  - После первого чтения директива помечается как `userDirectiveConsumed = true`, не забивая контекст в последующих циклах.

### 8.2 Live Activity Mini-Feed (Issue #84)
- **Концепция**: Хронологическая лента последних активностей агента внутри модального окна деталей (`enableLiveActivityFeed`, по умолчанию `true`).
- **События**: вызовы инструментов (`type: 'tool'`), обновление статусов вех (`type: 'milestone'`), git-коммиты чекпоинтов (`type: 'git'`), предупреждения бюджета (`type: 'budget'`), ручные паузы и директивы (`type: 'control'`).
- **Ограничение**: Автоматическая ротация до 10 последних записей для предотвращения утечек памяти.

### 8.3 Smart Goal Pre-planning (Issue #85)
- **Концепция**: Автоматическое предварительное структурирование задачи перед запуском основного цикла (`enablePreplanning`, по умолчанию `true`).
- **Поведение**:
  - При старте цели плагин анализирует заголовок и описание задачи с помощью эвристического планировщика (`generatePreplanDraft`) и формирует 3-4 предварительных этапа;
  - Цель ожидает подтверждения (`preplan.active = true`), в модальном окне отображается баннер с кнопкой «Утвердить и запустить план»;
  - Пользователь может отредактировать этапы или утвердить в 1 клик, после чего запускается автономный цикл.

### 8.4 Auto-Branching Workflow (Issue #86)
- **Концепция**: Автоматическое создание изолированной ветки git при старте новой цели (`autoBranchOnGoalStart`, по умолчанию `true`).
- **Формат ветки**: `goal/YYYYMMDD-<sanitized-slug>` (например, `goal/20260924-fix-auth-tokens`).
- **Управление**: По завершении или отмене цели в модальном окне появляются кнопки слияния (`merge`) в целевую ветку (`main`) или отказа (`discard`) с возвратом на исходную ветку.

### 8.5 Issue Checklist Sync (Issue #87)
- **Концепция**: Двусторонняя синхронизация с Markdown-чеклистами issues в Gitea и GitHub (`- [ ]` / `- [x]`).
- **Функционал**:
  - Автоматическое распознавание номера issue из заголовка цели (`#123` или полная URL);
  - Парсинг строк задач чеклиста и сопоставление с вехами плагина;
  - Обновление состояния чекбоксов в описании issue по мере прогресса этапов.

### 8.6 Post-Goal Retrospective Card (Issue #88)
- **Концепция**: Итоговая карточка аналитики выполнения, формируемая в момент завершения цели (`enablePostGoalRetrospective`, по умолчанию `true`).
- **Метрики**: общее время выполнения, чистый расход токенов, оценка стоимости по DeepSeek API, количество изменённых файлов (`git diff --stat`), число вызовов инструментов и процент ошибок инструментов (Tool Failure Rate), рекомендации по улучшению.

### 8.7 Smart Budget Auto-Scale (Issue #89)
- **Концепция**: Умное предотвращение срыва цели перед самым финишем из-за исчерпания токенов (`autoScaleBudgetNearCompletion`, по умолчанию `false`).
- **Алгоритм**: Если цель достигла 100% лимита токенов, но прогресс по вехам составляет >= 75% (или остался последний этап `completed === total - 1`), плагин однократно автоматически расширяет лимит на +20% буферных токенов и добавляет уведомление в системный промпт без аварийной остановки.

### 8.8 Goal Templates Drawer (Issue #90)
- **Концепция**: Библиотека типовых инженерных шаблонов целей (`enableTemplatesDrawer`, по умолчанию `true`).
- **Шаблоны**:
  - `refactor-module`: Рефакторинг и декомпозиция модуля до < 600 строк;
  - `bugfix-regression`: Локализация бага, написание регрессионного теста, фикс и приёмка;
  - `dsh-plugin-standard`: Аудит и приведение DSH-плагина к стандарту Cordis и i18n;
  - `security-preflight`: Проверка безопасности, CSRF, ограничений путей и payload;
  - `i18n-hygiene`: Очистка исходного кода от хардкод-локалей и проверка на отсутствие кириллицы.

### 8.9 Milestone Dependency Graph (Issue #91)
- **Концепция**: Поддержка явных зависимостей между вехами (`dependsOn: ['m-1']`) и топологический контроль выполнения (`enableMilestoneDependencies`, по умолчанию `true`).
- **Контроль**: Попытка перевести этап в статус `in_progress` или `completed` при незавершённых зависимостях блокируется; в системный промпт передаётся метка `[BLOCKED by ...]`, а в интерфейсе отображается замочек 🔒.

### 8.10 Sound Scheme Customization & Web Speech Announcements (Issue #92)
- **Звуковые схемы**: Настройка `soundScheme` со значениями `'default'` (пентатонические гармоники), `'minimal'` (мягкий одиночный колокольчик), `'retro'` (8-битные квадратные импульсы).
- **Голосовые оповещения**: Опция `enableVoiceAnnouncements` (по умолчанию `false`), озвучивающая завершение или ошибку цели («Goal completed: <title>») через Web Speech API (`window.speechSynthesis`).

---

## 9. Обновления безопасности и архитектурные фиксы (v0.2.11)

### 9.1 Защита маршрутов и SSE-потока от неавторизованных вызовов (Issue #78)
- Все GET-эндпоинты (`/dsh-goal/events`, `/dsh-goal/state`, `/dsh-goal/templates`) и POST `/dsh-goal/action` строго проверяются через `isTrustedCaller`.
- Заголовок `Sec-Fetch-Site: same-origin` более не принимается вслепую от внешних LAN-клиентов. Доступ разрешён только с loopback (127.0.0.1/::1) либо при наличии валидных `Origin`/`Referer`, совпадающих с заголовком `Host`.

### 9.2 Ограничение корневого каталога при старте цели (Issue #79)
- Параметр `cwd` при `action === 'start'` валидируется относительно доверенных корней (`process.cwd()` и `DSH_WORKSPACE_ROOT`). Попытки старта вне рабочей директории немедленно отклоняются с HTTP 400.
- Недоверенный `sessionGoal.cwd` исключён из самореференциального белого списка `allowedRoots` при операциях `save_artifact` и `rollback_milestone`.

### 9.3 Восстановление нативных инструментов ядра при выгрузке (Issue #94)
- При подмене базовых инструментов DSH (`get_goal`, `create_goal`, `update_goal`) и системной секции `tool:goal` исходные объекты сохраняются в snapshot-хранилище.
- При выгрузке плагина (disposal/HMR/перезагрузка) оригинальные инструменты и секция промпта восстанавливаются в глобальных слоях Cordis без утечек или потери функционала.

### 9.4 Активная синхронизация чеклистов с Gitea REST API (Issue #87)
- Функция `syncMilestoneWithIssue` при завершении вехи автономно обращается к Gitea REST API (`/api/v1/repos/{owner}/{repo}/issues/{number}`) с токеном из `.gitea-agent-credentials.json` и проставляет отметку `- [x]` в теле задачи.
